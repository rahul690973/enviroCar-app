...
public class LogbookFragment extends SherlockFragment implements
	...
	void createJSONFile() {
		...
		try {
			SharedPreferences settings = PreferenceManager
					.getDefaultSharedPreferences(getActivity());
			settings.edit()
					.putString(Default_Distance_Unit,
							Unit2.getText().toString()).commit();
			settings.edit()
					.putString(Default_Volume_Unit, Unit1.getText().toString())
					.commit();
			settings.edit()
					.putString(Default_Currency_Unit,
							Currency.getText().toString()).commit();
			...
			try {
				fueling.setCost(new NumberWithUOM(Double.parseDouble(Cost
						.getText().toString()), Currency.getText().toString()));
				fueling.setVolume(new NumberWithUOM(Double.parseDouble(Volume
						.getText().toString()), Unit1.getText().toString()));
				fueling.setMileage(new NumberWithUOM(Double
						.parseDouble(Distance.getText().toString()), Unit2
						.getText().toString()));
			...
			Log.i("JsonString :",
					new FuelingEncoder().createFuelingJson(fueling).toString());
	...
	public void makeToast(final String text) {
		getActivity().runOnUiThread(new Runnable() {
			...
			@Override
			public void run() {
				Toast.makeText(LogbookFragment.this.getActivity(), text,
	...
	private class MyAsyncTask extends AsyncTask<Fueling, Integer, Double> {
		...
		public void postData(Fueling valueIWantToSend) {
			// Create a new HttpClient and Post Header. Also HTTP-Adress from
			// Server
			// HttpClient httpclient = new DefaultHttpClient();
			// HttpPost httppost = new
			// HttpPost("http://ows.dev.52north.org/enviroCar/receiver.php");
			//
			// try {
			...
			// }
			try {
				DAOProvider.instance().getFuelingDAO()
						.storeFueling(valueIWantToSend);
